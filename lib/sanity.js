...
Sanity.API = (function (_Sanity, undefined) {
	...
	var getNodeStatus = function (region, callback, callbackError) {
		...
		sendRequest('POST', body, function (resp) {
			...
			if (json.queryContextResponse.contextResponseList) {
				status = json.queryContextResponse.contextResponseList.contextElementResponse.contextElement.contextAttributeList.contextAttribute.contextValue;
			} else {
				status = json.queryContextResponse.errorCode.code;
			}
